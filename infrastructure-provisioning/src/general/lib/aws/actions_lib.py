...
def prepare_disk(os_user):
    if not exists('/home/' + os_user + '/.ensure_dir/disk_ensured'):
        try:
            ...
            sudo('mkfs.ext4 -F /dev/{}p1'.format(disk_name))
            sudo('mount /dev/{}p1 /opt/'.format(disk_name))
            sudo(''' bash -c "echo '/dev/{}p1 /opt/ ext4 errors=remount-ro 0 1' >> /etc/fstab" '''.format(disk_name))
